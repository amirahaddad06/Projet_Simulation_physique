...
class Particule(object):
    ...
    def __str__(self):
        msg = f'Particule ({self.mass}, {self.position[-1]}, {self.speed[-1]}, {self.acceleration[-1]}, "{self.name}", "{self.color}")'
        return msg
    ...
    def plot(self):
        ...
        X = [p.x for p in self.position]
        Y = [p.y for p in self.position]
    ...
    def gameDraw(self, scale, screen):
        ...
        # uniquement si la particule est mobile, tracer la vitesse
        if not self.fix:
